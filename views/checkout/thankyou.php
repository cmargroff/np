...
<!------------------------------------------------------------------------------------------------------------- Account confirmation  -------------------------------------------------------------------------------------------------->
	...
	<div class="receipt">
		<h2>Order Receipt <span class="order-id">Order #: <strong><?php echo $cart->id_str?></strong></span></h2>

		<?php $this->renderPartial('_orderdetails',array('cart' => $cart)); ?>
...
//	                                        "onblur" =>"updateCart(this, $data->qty)",
						...
						array(
							'type' => 'raw',
							'value' => 'CHtml::link("Share","#",array(
									"data-pk"=>$data->id,"class"=>"edit", "onclick"=> "showShareCartModal();")
									).CHtml::link("&times;","#",array("class"=>"remove"))',
							'htmlOptions' => array(
								'class' => 'controls'
							),
...
<script>
	function showShareCartModal() {
		$('.sharecartmodal:first').addClass('show');
	}
</script>
